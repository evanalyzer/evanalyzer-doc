...
In image processing, a _kernel_ (convolution matrix) is a small matrix applied to each pixel and its neighbours. A box blur kernel fills every element with 1/N, replacing each pixel with the neighbourhood mean.
